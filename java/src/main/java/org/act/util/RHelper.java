...
/**
 * Includes Helper methods to run simulation from R.
 */
public class RHelper {
	 ...
	 *                             defining enemy item constraints through the preclude 
	 *                             column                        
	 ...
	 * @param numItemPerPassageLB  the lower bound of number of items per passages in the test
	 * @param numItemPerPassageUB  the upper bound of number of items per passages in the test
	 * @param simID 			   a string that specifies the simulation identifier
	 * @param numExaminees         the number of simulated examinees
	 * @param trueThetaDistType    a string that specifies the type of distribution of true ability theta
	 * @param trueThetaDistParams  an array that specifies the parameters of the true theta distribution 
    ...
    public static List<SimOutput> runSim(
            ...
            double initTheta, double scalingConstant,String exposureControlType, double rMax, int lValue,
            double absGap, double relGap, double intTol, boolean saveInput,
            String testConfigID, int testLength,String itempoolPath, String passagepoolPath, String constraintPath,
            boolean[] itemNumericColumn, boolean[] passageNumericColumn, boolean enableEnemyItem,
            int numPassageLB, int numPassageUB, int numItemPerPassageLB, int numItemPerPassageUB,
            String simID,int numExaminees, String trueThetaDistType, double[] trueThetaDistParams
        ...
        List<ThetaRange> thetaRanges = Arrays.asList(new ThetaRange(ExposureControlFunctions.EC_THETA_MIN, 
        ...
        CatConfig catConfig = new CatConfigStandard(solverConfig, initTheta, scalingConstant,
                scoringMethodConfig, exposureConfig, lValue);

        // Initialize test configuration
        ContentTable.RowOriented itemPoolTable = CsvUtils
                .read(new FileInputStream(new File(itempoolPath)));
        ...
        AbstractCatSimulation catSim = new CatSimulationStandard(simID, numExaminees,
                ProbDistributionFactory.getProbDistribution(trueThetaDistType, trueThetaDistParams),
                testConfig, catConfig, true);
